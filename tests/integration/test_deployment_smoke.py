...
class TestDeploymentBasics:
    ...
    def test_airflow_home_exists(self):
        ...
        assert os.path.exists(airflow_home), \
            f"Airflow home directory not found: {airflow_home}"

    def test_dags_folder_exists(self):
        """Test that DAGs folder exists and is accessible."""
        assert os.path.exists(DAGS_FOLDER), \
            f"DAGs folder not found: {DAGS_FOLDER}"
        assert os.path.isdir(DAGS_FOLDER), \
            f"DAGs path is not a directory: {DAGS_FOLDER}"
...
class TestDAGDeployment:
    ...
    def test_all_dags_have_owners(self, dag_bag):
        ...
        assert len(dags_without_owners) == 0, \
...
class TestDatabaseConnectivity:
    ...
    def test_metadata_db_accessible(self):
        ...
        try:
            ...
            result = session.query(DagModel).limit(1).all()
            session.close()
